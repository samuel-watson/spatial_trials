...
generate_intervention <- function(data, max_dist, beta, n_locs, plot = TRUE){
  ...
  data$fn <- fun(data$distance,50,4,8,max_dist,TRUE)
...
for(zz in 1:1000){
  ...
  if(zz %% 100){
    saveRDS(pvals,paste0(getwd(),"/results/pvals_b0_mis.RDS"))
    saveRDS(pvals_ml,paste0(getwd(),"/results/pvals_ml_b0_mis.RDS"))
  }
}
...
for(i in 1:nrow(dfci)){
  ...
  dfanal$fn <- fun(dfanal$distance,50,4,8,best_del[1],TRUE)
  ...
  rm(pt)
  if(i %% 10 == 0){
    saveRDS(dfci,paste0(getwd(),"/results/dfci_b03_cont_mis.RDS"))
    saveRDS(dfcid,paste0(getwd(),"/results/dfcid_b03_cont_mis.RDS"))
  }
  
}

saveRDS(dfci,paste0(getwd(),"/results/dfci_del3_3_mis.RDS"))
saveRDS(dfcid,paste0(getwd(),"/results/dfcid_del3_3_mis.RDS"))
