...
genrep <- function(dfanal,f1,L,r){
  dfanal$ystar <- f1 + L%*%(rnorm(length(f1))) #sr2[sample(1:length(sr2),replace = TRUE)]
  # e <- sample(c(1,-1),length(f1),replace=TRUE)
  # dfanal$ystar <- f1 + L%*%(e*r)
  # w <- (L%*%rnorm(length(f1)))
  # dfanal$ystar <- f1 + r*w
