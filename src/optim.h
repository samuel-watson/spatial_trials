...
// class for L-BFGS
template <>
class optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS> {
  using func = double(*)(void*, const Eigen::VectorXd&, Eigen::VectorXd&); 
public:
  struct optimControl {
  ...
  optim(const Eigen::VectorXd& start);
  optim(const optim& x) = default;
  auto operator=(const optim& x) -> optim& = default;
  
  // functions
  // the two functions fn() enable capturing of functions to use in the algorithm
  template<auto Function, typename = std::enable_if_t<std::is_invocable_r_v<double, decltype(Function), const Eigen::VectorXd&, Eigen::VectorXd& > > >
  void                fn();
  template<auto Function, typename Class, typename = std::enable_if_t<std::is_invocable_r_v<double, decltype(Function), Class*, const Eigen::VectorXd&, Eigen::VectorXd& > > >
  void                fn(Class* cls);

  auto                operator()(const Eigen::VectorXd& vec, Eigen::VectorXd& g) -> double;
  void                minimise(); // the optim algorithm
...
private:
  [[noreturn]]
  static auto null_fn(void* p, const Eigen::VectorXd& x, Eigen::VectorXd& g) -> double {throw std::exception{};}
  ...
  Eigen::VectorXd         lower_bound;               // bounds
  ...
  //functions
  double           eval(const Eigen::VectorXd& vec, Eigen::VectorXd& g);
};


inline optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::optim(const Eigen::VectorXd& start) : dim(start.size()), current_values(start), 
  lower_bound(dim), upper_bound(dim) {};

inline Eigen::VectorXd optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::values() const 
{
  return current_values;
}

inline void optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::set_bounds(const Eigen::VectorXd& lower, const Eigen::VectorXd& upper)
{
  for(int i = 0; i < dim; i++)
  {
...
inline void optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::set_bounds(const std::vector<double>& lower, const std::vector<double>& upper)
{
...
template <auto Function, typename>
inline void optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::fn() 
{
  optim_instance = nullptr;
  optim_fn = static_cast<func>([](void*, const Eigen::VectorXd& x, Eigen::VectorXd& g) -> double {
    return std::invoke(Function, x, g);
  });
};

template<auto Function, typename Class, typename>
inline void optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::fn(Class* cls)
{
  optim_instance = cls;
  optim_fn = static_cast<func>([](void* p, const Eigen::VectorXd& x, Eigen::VectorXd& g) -> double {
    auto* c = static_cast<Class*>(p);
    return std::invoke(Function,c,x,g);
  });
}

inline auto optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::operator()(const Eigen::VectorXd& vec, Eigen::VectorXd& g) -> double
{
  fn_counter++;
  return std::invoke(optim_fn,optim_instance,vec,g);
}   

inline double optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::eval(const Eigen::VectorXd& vec, Eigen::VectorXd& g) 
{
  return std::invoke(optim_fn,optim_instance,vec,g);
}   

inline void optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS>::minimise()
{
        int niter;
        fn_counter = 0;
        if(!bounded){
...
typedef optim<double(const Eigen::VectorXd&, Eigen::VectorXd&),LBFGS> lbfgsd;
